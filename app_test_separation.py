...
from tts_module import (
    generate_audio_for_subtitles,  # GPT 배정 화자 지원 + 나래이션/대사 분리
    add_audio_to_video,
...
if mode == "(기존) 이미지 선택 기반 제작":
    st.info(" 마음에 드는 삽화를 먼저 고르면, AI가 이야기를 이어줍니다.")
    # --------------------------------
    # ① 삽화 선택
    # --------------------------------
    ...
    # BGM 폴더 자동 매칭 (BGM/ 하위 폴더명과 유사도 비교)
    ...
    **캐릭터 음성** - 오직 직접 대사(따옴표 안)만!
        ...
        for item in subtitles:
            if isinstance(item, dict):
                ...
                if speaker not in VALID_SPEAKERS:
                    print(f"예상치 못한 화자 '{speaker}' → narrator로 변경")
                    speaker = "narrator"
    ...
    # [STEP 1] 대본 초안 생성 (GPT)
    # =========================================================
    st.markdown("#### 1️ 대본(Script) 생성 및 수정")
    ...
    if st.session_state.step1_scripts is not None:
        ...
        # [STEP 2] TTS 음성 생성
        # =========================================================
        st.markdown("#### 2️ TTS 음성 생성 및 미리듣기")
        ...
        if st.button("2단계: 수정된 대본으로 TTS 생성", type="primary"):
            ...
            st.info(" TTS 음성을 생성하고 길이를 측정합니다...")
            ...
            for i, vid in enumerate(video_paths):
                ...
                if audio and os.path.exists(audio):
                    ...
                    if page_bgm and page_bgm.exists():
                        ...
                        print(f"   Scene {i+1} ({img_name}): BGM '{page_bgm.name}' 적용")
                    else:
                        add_audio_to_video(sub_out, audio, final_out)
                        if use_bgm:
                            print(f"   Scene {i+1} ({img_name}): 매칭되는 BGM 없음")
                else:
