...
def add_audio_to_video(video_path: str, audio_path: str, output_path: str, bgm_path: str = None, bgm_volume: float = 0.15) -> bool:
    ...
    try:
        ...
        target_duration = video.duration  # 원본 영상 길이 보존
        ...
        if bgm_path and Path(bgm_path).exists():
            ...
            # BGM을 영상 길이에 맞춤 (짧으면 반복, 길면 자르기)
            if bgm.duration < target_duration:
                num_loops = int(target_duration / bgm.duration) + 1
                bgm_loops = [bgm] * num_loops
                bgm = concatenate_audioclips(bgm_loops).subclip(0, target_duration)
            else:
                bgm = bgm.subclip(0, target_duration)

            # TTS + BGM 합성 (duration을 영상 길이로 고정)
            final_audio = CompositeAudioClip([audio, bgm]).set_duration(target_duration)
            final = video.set_audio(final_audio)
        else:
            # TTS가 영상보다 짧을 경우 → 나머지 구간은 무음 처리
            final_audio = CompositeAudioClip([audio]).set_duration(target_duration)
            final = video.set_audio(final_audio)
...
def concat_videos_with_audio(video_paths: list, output_path: str):
    """
    ...
    try:
        ...
        # 합치기 전 개별 파일 존재 여부 확인
        missing = [p for p in video_paths if not os.path.exists(str(p))]
        if missing:
            msg = f"누락된 클립 파일: {missing}"
            print(f"  ❌ {msg}")
            return msg

        empty = [p for p in video_paths if os.path.getsize(str(p)) == 0]
        if empty:
            msg = f"비어있는 클립 파일(0 bytes): {empty}"
            print(f"  ❌ {msg}")
            return msg
