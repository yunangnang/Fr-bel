...
def get_bgm_folder_name(full_name: str) -> str:
    ...
    for key, value in bgm_mapping.items():
        if key.replace(" ", "") in full_name.replace(" ", "").replace("_", ""):
            return value
    return full_name
...
def get_bgm_for_page(page_name: str, bgm_dir: Path):
    ...
    # BGM 폴더에서 해당 페이지 번호의 파일 찾기 (예: "xxx_11P_BGM.wav")
    for bgm_file in bgm_dir.iterdir():
        if bgm_file.suffix.lower() not in ['.wav', '.mp3']:
            continue
        # BGM 파일명에서 페이지 번호 추출 (예: "_11P" -> 11)
        ...
        if st.session_state.get("track_b_step", 0) >= 7:
            ...
            with col_prev_opt:
                ...
                if use_bgm:
                    if BGM_DIR.exists():
                        bgm_files = sorted(
                            [f for f in BGM_DIR.iterdir() if f.is_file() and f.suffix.lower() in ['.mp3', '.wav']],
                            key=lambda x: x.name
                        )
                ...
                if not matches or not audios:
                    ...
                    try:
                        ...
                        for i, match in enumerate(matches):
                            ...
                            # D. 영상 생성 (무음)
                            clip = ImageClip(img_path).resize(height=1280)
                            current_width = clip.w
                            clip = crop(clip, x_center=current_width/2, width=720, height=1280)
                            clip = clip.set_duration(audio_dur).set_fps(24)
                            ...
                            if audio_path and os.path.exists(audio_path):
                                ...
                                if use_bgm and source_page >= 0:
                                    ...
                                    # [Logic] 표지보다 뒤쪽 페이지인 경우 -> 상대 번호 + "P" 검색
                                        ...
                                        search_keywords = [f"{story_seq}p", f"{story_seq}P"]
                                ...
                                if page_bgm and page_bgm.exists():
                                    ...
                                    print(f"  🎵 Scene {i+1} (PDF {source_page}p): [{log_msg}] 조건으로 '{page_bgm.name}' 적용됨")
                                else:
                                    add_audio_to_video(
                                        str(sub_clip_path), str(audio_path), str(final_clip_path)
                                    )
            ...
            if st.button(btn_label, type="primary"):
                ...
                try:
                    for i, vid_data in enumerate(video_results):
                        ...
                        if audio_path and os.path.exists(audio_path):
                            ...
                            if page_bgm and page_bgm.exists():
                                ...
                                print(f"  🎵 Scene {i+1}: BGM '{page_bgm.name}' ({log_msg}) 적용")
                            else:
                                add_audio_to_video(
                                    temp_sub_path,
                                    str(audio_path),
                                ...
                                if use_bgm:
                                    print(f"   Scene {i+1}: 매칭되는 BGM 없음 (Source: {source_page}p)")

                        else:
                           
                            shutil.move(temp_sub_path, str(output_clip_path))
                    ...
                    if final_clips_paths:
                        ...
                        concat_videos_with_audio(final_clips_paths, str(final_movie_path))
